  ...
  return allFacts.reduce(
    (
      ...
      if (allFilters.search) {
        ...
        if (
          !highlightFact &&
          allFilters.searchOptions?.includes(0) &&
          fact.tag
        ) {
        ...
        if (highlightFact) {
          accumulator.highlight.push(fact.id as string);
        }
      }
      if (isFilterActive) {
        accumulator.isFilterActive = true;
        let activateFact = 0;
        ...
        if (activateFact) {
          accumulator.active.push(fact.id as string);
        }
      }
      return accumulator;
    },
