...
export default class SectionsTable extends FilingSpecific {
    ...
    async parseSectionsData(input: DataJSON) {
        ...
        for await (const current of input['ixv:edgarRendererReports']) {
            ...
            const contextRef = current[`ixv:uniqueAnchor`] ? current[`ixv:uniqueAnchor`][`ixv:contextRef`] : null;
            const name = current[`ixv:uniqueAnchor`] ? current[`ixv:uniqueAnchor`][`ixv:name`] : null;
