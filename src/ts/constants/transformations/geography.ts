export const TransformationsGeography = {
  countryNameEn: (input: string) => {
    if (input) {
      const countryCode = {
      ...
      for (const current in countryCode)
        if (
          countryCode[current].exec(input) &&
          countryCode[current].exec(input)[0]
        )
          return current;
    }
    return "Format Error: Country Name EN";
  },
  stateProvNameEn: (input: string) => {
    if (input) {
      const stateProvNames = {
        AL: /^\s*([Aa]labama)\s*$/,
        AK: /^\s*([Aa]laska)\s*$/,
        AZ: /^\s*([Aa]rizona)\s*$/,
        AR: /^\s*([Aa]rkansas)\s*$/,
      ...
      for (const current in stateProvNames)
        if (
          stateProvNames[current].exec(input) &&
          stateProvNames[current].exec(input)[0]
        ) {
          return current;
        }
    }
  ...
  edgarProvCountryEn: (input: string) => {
    if (input) {
      const provCountry = {
      ...
      for (const current in provCountry)
        if (
          provCountry[current].exec(input) &&
          provCountry[current].exec(input)[0]
        ) {
          return current;
        }
    }
