...
export class Nested extends BaseModal {
    ...
    if (name === `fact-id` && newValue) {
      ...
      if (element && element?.parentElement) {
        const nestedIDs = Array.from(
          element?.parentElement?.querySelectorAll(`[fact]`)
        ).map((current, index) => {
          return {
        ...
        BaseModal.prototype.init.call(
          this,
          ["Company and Document", "Tags", "Files", "Additional Items"],
          nestedIDs
        );
